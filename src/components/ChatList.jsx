...
class ChatList extends Component {
    ...
    startChatting = () => {
        if(isEmailValid(this.state.searchedUserEmail)) {
            fetch(process.env.REACT_APP_BACKEND_API_URL + "/users?email=" + this.state.searchedUserEmail, {
                method: "GET",
                headers: {
                    "Authorization": `Bearer ${this.props.accessToken}`
                }
            ...
            <div className={"chat-list" + (this.props.isChatOpen ? " chat-list--closed" : "")}>
                <div className="chat-list__wrapper">
                    <div className="chat-list__top">
                        ...
                        <button className="button primary-button chat-list__new-chat" onClick={this.toggleDialog}>
                            <img src={iconPlus} />
                            <span>new chat</span>
                        </button>
                    </div>
